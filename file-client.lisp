  ...
  (let ((num-direct (num-of-direct-block)) (num-of-entries (/ *block-size* 4)))
    (cond ((< index num-direct) :direct)
	  ((< index (+ num-direct (* num-of-entries *num-of-l1-indirect*)))
	   (values :indirect (list (+ num-direct (floor (- index num-direct) 
							num-of-entries)) 
				   (mod (- index num-direct) num-of-entries))))
	  ((< index (+ num-direct 
		       (* num-of-entries *num-of-l2-indirect*) 
		       (* (expt num-of-entries 2) *num-of-l2-indirect*)))
	   (let ((entries-per-l2 (expt num-of-entries 2)))
	     (values :indirect
		     (list (+ num-direct 
			      *num-of-l1-indirect* 
			      (floor (- index num-direct 
					(* num-of-entries *num-of-l1-indirect*)) 
				     entries-per-l2))
			   (floor (mod (- index num-direct 
					  (* num-of-entries *num-of-l1-indirect*))
				       entries-per-l2) 
				  num-of-entries)
			   (mod (- index num-direct 
				   (* num-of-entries *num-of-l1-indirect*)) 
				num-of-entries)))))
	  (t nil))))
...
(defun hop-and-swap (ec block-num hop-list new-val)
  (cond ((eql nil (cdr hop-list)) 
	 (let ((content (get-block ec block-num)) (old-val 0))
	...
	(t (let* ((content (get-block ec block-num)) 
		  (next-level (load-int32-from-string content (* 4 (car hop-list)))))
...
(defmethod set-block-ptr ((client file-client) index block-num)
  (multiple-value-bind (index-type index-list) (block-ptr-type index)
    (let ((old-block-num 
	     (cond ((eql :direct index-type) 
		    (let ((old-block-num (nth index (entries client))))
		      (setf (nth index (entries client)) block-num)
		      old-block-num))
		   ((eql :indirect index-type) 
		    (progn 
...
(defun hop-and-find (ec block-num hop-list)
  (cond ((eql nil (cdr hop-list)) 
	 (let ((content (get-block ec block-num)))
...
(defmethod write-content ((client file-client) offset size content)
  (cond ((< (length content) size) nil)
	(t (let ((ptr-index 0) (block-num -1) (block-content nil))
	     (do ((size-count 0 (1+ size-count))
		  (pos-in-str 0 (1+ pos-in-str))
		  (pos-in-block (mod offset *block-size*) 
				(mod (1+ pos-in-block) *block-size*)))
		 ((>= size-count size) (put-block (extent-client client) 
						  block-num block-content))
	       (when (or (eql 0 pos-in-block) (eql 0 size-count))
		 (unless (eql -1 block-num)
		   (put-block (extent-client client) block-num block-content))
		 (setf ptr-index (floor (+ size-count offset) *block-size*))
		 (setf block-num (get-block-ptr client ptr-index))
		 (setf block-content (get-block (extent-client client) block-num)))
	       (setf (char block-content pos-in-block) (char content pos-in-str)))))))

(defmethod read-content ((client file-client) offset size)
  (let ((ptr-index 0) (block-num -1) (block-content nil) 
	(content (make-string size)))
    (do ((size-count 0 (1+ size-count))
	 (pos-in-str 0 (1+ pos-in-str))
	 (pos-in-block (mod offset *block-size*) 
		       (mod (1+ pos-in-block) *block-size*)))
	((>= size-count size) content)
      (when (or (eql 0 pos-in-block) (eql 0 size-count))
	(unless (eql -1 block-num)
	  (put-block (extent-client client) block-num block-content))
	(setf ptr-index (floor (+ size-count offset) *block-size*))
	(setf block-num (get-block-ptr client ptr-index))
	(setf block-content (get-block (extent-client client) block-num)))
      (setf (char content pos-in-str) (char block-content pos-in-block)))))
